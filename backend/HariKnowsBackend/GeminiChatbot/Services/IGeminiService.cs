...
using HariKnowsBackend.GeminiChatbot.Repositories;
using Google.GenAI;
...
namespace HariKnowsBackend.GeminiChatbot.Services;
...
public sealed class GeminiService(IConfiguration config, ILogger<GeminiService> logger) : IGeminiService
{
    ...
    private readonly string _systemPrompt = config["GeminiApi:SystemPrompt"] ?? GetDefaultSystemPrompt();
    ...
    public async Task<string> GetChatResponseAsync(string userMessage, IReadOnlyList<ChatResponseDto> conversationHistory)
    {
        ...
        try
        {
            var response = await GenerateWithRetryAsync(contents, generationConfig);
            return response;
        }
        finally
        {
    ...
    {
        return @"You are a helpful and professional registrar helpdesk assistant. Your role is to provide accurate information about student records, university policies, academic procedures, and general registrar services. Please be courteous, clear, and knowledgeable in your responses. If you're unsure about something, acknowledge it and offer to help find the correct information.";
    }
